...
6. Run .m for OMP and CG-CGAN channel estimation frameworks to calculate NMSE performances. It depends on [OMP_weight.m](OMP_weight.m) and functions to perform LS estimations.  The MATLAB evaluation script loads outputs/rand_ind_ch.mat, a saved set of 1000 random channel indices (sampled from non-zero channels in the test split). This ensures all reported numbers can be reproduced exactly. Do not delete or regenerate this file; users running the same script with the same random indices will get the same NMSE values to within floating-point noise. 

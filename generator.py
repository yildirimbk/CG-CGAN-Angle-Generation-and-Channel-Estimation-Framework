...
def generate_data(ext_params, path):
    
    np.random.seed(1001)
    
    params = validate_params(copy.deepcopy(ext_params))
    
    #print('generator_params:',params)
    print('path in generator:',path)
    #path = 'r' + '\'' + path + '\''
    ...
    for i in range(num_active_bs):
        ...
        if params['scenario_params']['dual_polar_available'] and params['enable_dual_polar']:
            for polar_str in ['VV', 'VH', 'HH', 'HV']:
                dataset[i][c.DICT_UE_IDX][polar_str][c.OUT_CHANNEL], dataset[i][c.DICT_UE_IDX][polar_str][c.OUT_LOS], dataset[i][c.DICT_UE_IDX][polar_str][c.OUT_PHASE_TERM] = generate_MIMO_channel(dataset[i][c.DICT_UE_IDX][polar_str][c.OUT_PATH], 
                                                                                                                                             params, 

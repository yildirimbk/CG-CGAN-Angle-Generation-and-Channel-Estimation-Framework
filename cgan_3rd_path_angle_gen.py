#v1: 3rd path 4 angles CGAN model
import torch
...
label_size = 11-1 #  2 user loc, 2bs loc and distance, azimuthal angle, elevation angle, and num_paths(length 2 vector)
...
g_base_lr = 1e-3
d_base_lr = 1e-3
checkpoint_path = "chkpt_cgan_angle_gen_4path_3p_v1.pth"  # Path to save/load checkpoints
best_checkpoint_path = "best_chkpt_cgan_angle_gen_4path_3p_v1.pth"
matfile_path = "losses_cgan_angle_gen_4path_3p_v1.mat"  # Path to save/load checkpoints

#                                           ### DATA PREPARATION ###
training_data1_path = '/main/training_dataset_3_morepaths_80percent_k1_no_path_removed.mat' #change path with the current folder path that you are saving the dataset
...
val_data1_path = '/main/validation_dataset_3_morepaths_10percent_k1_no_path_removed.mat'  #change path with the current folder path that you are saving the dataset
...
test_data1_path = '/main/test_dataset_3_morepaths_10percent_k1_no_path_removed.mat' #change path with the current folder path that you are saving the dataset
...
def Custom_Dataset(dataset_type):
    outputs=dataset_type[:,[9,13,17,21]].astype(np.float32)
...
if activate_training:
    ...
    joblib.dump(preprocessor, "mixed_label_scaler_3rd_CGAN.pkl")

    if use_minmax_scaler_outputs:

        joblib.dump(s_sc_outputs, "s_sc_outputs_minmaxscaler_3rd_CGAN.pkl")

    else:

        joblib.dump(s_sc_outputs, "s_sc_outputs_stdscaler_3rd_CGAN.pkl")

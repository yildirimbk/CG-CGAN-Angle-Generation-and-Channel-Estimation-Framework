# Original upstream location: DeepMIMOv3/construct_deepmimo.py (in site-packages of the DeepMIMOv3 package).
# This is a modified copy. Differences from upstream:
#   - Skips LoS-status tracking for the generated rt outputs (used during inference where
#     the LoS flag is not propagated through the CGAN pipeline).
#   - Returns both path_const and path_const2 from OFDM_PathGenerator.no_LPF.
...
# if LoS status of all paths will be used as a parameter, length of rayt_output is 9, otherwise 8
len_parameter =8 #originally it is 9  
...
def generate_MIMO_channel(raydata, params, tx_ant_params, rx_ant_params):
    #print('ray data:')
    #print('length ray data:',len(raydata))
    #print(params)
    #print('tx_ant_params:')
    #print(tx_ant_params)
    rx_ant_params_rot = np.zeros((len(raydata), 3), dtype=np.float32) #add this and below line to not get error
    rx_ant_params[c.PARAMSET_ANT_ROTATION]=rx_ant_params_rot
    #print('rx_ant_params_rot:')
    #print(rx_ant_params[c.PARAMSET_ANT_ROTATION])
    #print(rx_ant_params[c.PARAMSET_ANT_ROTATION].shape)
    #sys.exit()
    #print(('range length ray data:',range(len(raydata))))
    #print('range length ray data single:',range(1))
    #print(raydata[0])
    #print(raydata)
    #print('params:')
   
    #print('tx_Ant_spacing',tx_ant_params[c.PARAMSET_ANT_SPACING])
    #print('tx_Ant_spacing',tx_ant_params['spacing'])
    ...
    if  params[c.PARAMSET_FDTD]:
        if len(raydata)==len_parameter:
            channel = np.zeros((1, M_rx, M_tx, len(subcarriers)), dtype = np.csingle)
        else:
            channel = np.zeros((len(raydata), M_rx, M_tx, len(subcarriers)), dtype = np.csingle)  
            path_const2 = [None] * len(raydata)  # Initialize as a list
    else:
        if len(raydata)==len_parameter:
            channel = np.zeros((1, M_rx, M_tx, params[c.PARAMSET_NUM_PATHS]), dtype = np.csingle)
        else:
    ...
    if len(raydata)==len_parameter:
        ...
        dod_theta, dod_phi = rotate_angles(rotation = tx_ant_params[c.PARAMSET_ANT_ROTATION],
                                    theta = raydata[c.OUT_PATH_DOD_THETA],
                                    phi = raydata[c.OUT_PATH_DOD_PHI])
            
        doa_theta, doa_phi = rotate_angles(rotation = rx_ant_params[c.PARAMSET_ANT_ROTATION][0], #180
                                    theta = raydata[c.OUT_PATH_DOA_THETA],
                                    phi = raydata[c.OUT_PATH_DOA_PHI])
        ...
        for key in raydata.keys():
            if key == 'num_paths':
                raydata[key] = FoV.sum()
            else:
                raydata[key] = raydata[key][FoV]
        ...
        power = antennapattern.apply(power = raydata[c.OUT_PATH_RX_POW], 
                                        doa_theta = doa_theta, 
                                        doa_phi = doa_phi, 
                                        dod_theta = dod_theta, 
                                        dod_phi = dod_phi)
        raydata[c.OUT_PATH_RX_POW] = power
            
        if  params[c.PARAMSET_FDTD]: # OFDM
            path_const = path_gen.generate(raydata, Ts)
                
            # The next step is to be defined
            if params[c.PARAMSET_OFDM][c.PARAMSET_OFDM_LPF] == 0:
                channel = np.sum(array_response_RX[:, None, None, :] * array_response_TX[None, :, None, :] * path_const.T[None, None, :, :], axis=3)
            else:
                channel = np.sum(array_response_RX[:, None, None, :] * array_response_TX[None, :, None, :] * path_const.T[None, None, :, :], axis=3) @ path_gen.delay_to_OFDM
            
        else: # TD channel
            channel[:, :, :raydata[c.OUT_PATH_NUM]] = array_response_RX[:, None, :] * array_response_TX[None, :, :] * (np.sqrt(power) * np.exp(1j*np.deg2rad(raydata[c.OUT_PATH_PHASE])))[None, None, :]
        ...
        for i in tqdm(range(len(raydata)), desc='Generating channels'):
            ...
            array_response_TX_transpose = array_response_TX.transpose()
            ...
            if  params[c.PARAMSET_FDTD]: # OFDM
                path_const,path_const2[i] = path_gen.generate(raydata[i], Ts)
...
def generate_MIMO_channel_rx_ind(raydata, params, tx_ant_params, rx_ant_params):
    ...
    if len(raydata)==9:     
        for i in tqdm(range(1), desc='Generating channels'):
            ...
            if raydata['num_paths'] == 0:
                LoS_status.append(-1)
            else:
                LoS_status.append(raydata['LoS'].sum())
            ...
            if  params[c.PARAMSET_FDTD]: # OFDM
                ...
                channel.append(array_response_RX[:, None, :] * array_response_TX[None, :, :] * (np.sqrt(power) * np.exp(1j*np.deg2rad(raydata[c.OUT_PATH_PHASE])))[None, None, :])
...
class OFDM_PathGenerator:
    ...
    def no_LPF(self, raydata, Ts):
        ...
        path_const2 = np.sqrt(power/self.total_subcarriers) * np.exp(1j*(np.deg2rad(phase) - (2*np.pi/self.total_subcarriers)*np.outer(delay_n, self.subcarriers) ))

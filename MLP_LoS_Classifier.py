...
checkpoint_path      = os.path.join(OUTPUT_DIR, "chkpt_los_classifier.pth")
best_checkpoint_path = os.path.join(OUTPUT_DIR, "best_chkpt_los_classifier.pth")
matfile_path         = os.path.join(OUTPUT_DIR, "losses_los_classifier.mat")

### DATA PREPARATION ###
training_data_path = os.path.join(INPUT_DIR, f'training_dataset_{RUN_TAG}.mat')
val_data_path      = os.path.join(INPUT_DIR, f'validation_dataset_{RUN_TAG}.mat')
...
def split_dataset(dataset):
    """Extract input features (BS x,y, UE x,y) and LoS labels."""
    los_status = dataset[:, 5].astype(np.float32).reshape(-1, 1)
    labels     = dataset[:, 0:4].astype(np.float32)  # BS_x, BS_y, UE_x, UE_y
    return labels, los_status

train_labels, train_los = split_dataset(train_data)
val_labels,   val_los   = split_dataset(val_data)
test_labels,  test_los  = split_dataset(test_data)

# Shift class labels from {-1, 0, 1} to {0, 1, 2}
train_los_shifted = train_los + 1
val_los_shifted   = val_los + 1
test_los_shifted  = test_los + 1
...
train_dataset = np.concatenate((train_labels, train_los_shifted), axis=1)
val_dataset   = np.concatenate((val_labels,   val_los_shifted),   axis=1)
test_dataset  = np.concatenate((test_labels,  test_los_shifted),  axis=1)

train_data_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,  drop_last=True, num_workers=32) #tune num_workers to your system
val_data_loader   = DataLoader(val_dataset,   batch_size=batch_size, shuffle=False, drop_last=True, num_workers=32) #tune num_workers to your system
test_data_loader  = DataLoader(test_dataset,  batch_size=batch_size, shuffle=False, drop_last=True, num_workers=32) #tune num_workers to your system
...
### INFERENCE ###
    ...
    with torch.no_grad():
        for batch in tqdm(test_data_loader, desc="Evaluating LoS classifier"):
            inputs = batch[:, :input_size].to(device).float()
            labels = batch[:, input_size].to(device).long()
    ...
    target_names = [f"Class {i}" for i in range(output_size)]
    print("\nLoS Classifier Results on Test Set:")
    print(f"Accuracy: {final_accuracy * 100:.2f}%")
    print(f"\nClassification report:\n{classification_report(all_labels, all_predictions, target_names=target_names)}")
    ...
    plt.title(f"LoS Classifier Confusion Matrix (Acc: {final_accuracy*100:.2f}%)")
    plt.savefig(os.path.join(OUTPUT_DIR, "los_confusion_matrix.eps"), format='eps', bbox_inches='tight')
    plt.show()
